...
:END:
[[https://www.zotero.org][Zotero]] exposes a local HTTP API on =http://127.0.0.1:23119= (requires the
...
their citation keys for completion.  It requires the
[[https://retorque.re/zotero-better-bibtex/][Better BibTeX]] Zotero plugin, which adds a stable =citekey= field to each
item, and caches results so repeated completion calls are cheap.
...
function ZoteroSource:get_items()
  -- Better BibTeX search endpoint — returns JSON array of {citekey, ...}
  local url = 'http://127.0.0.1:23119/better-bibtex/cayw?format=json&all=1'
...
For a simpler approach that does not require Better BibTeX, query the standard
Zotero items endpoint and derive a key from the built-in Zotero item key:

#+begin_src lua
local ZoteroSimpleSource = {}

function ZoteroSimpleSource:get_name()
  return 'zotero_simple'
end

function ZoteroSimpleSource:get_items()
  -- Replace 0 with your numeric Zotero user ID if needed.
  -- Increase the limit or add ?start= pagination for large libraries.
  local url = 'http://127.0.0.1:23119/api/users/0/items?format=json&limit=100'
  local ok, result = pcall(vim.fn.system, { 'curl', '-s', url })
  if not ok or vim.v.shell_error ~= 0 then
    return {}
  end
